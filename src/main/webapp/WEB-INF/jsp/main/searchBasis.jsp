			...
			$.ajax({
				...
				success : function(result){  
 					$.each(result, function(idx, floorList) {
 						...
 						insertTr += "<td align='right'>" + floorList.flrArea + "&nbsp;평</td>";
...
<div class="container mb-5">
	<h5  class="pt-4 mb-4" style="color:#004080;margin-bottom:20px;text-align:center;"><strong>건축인허가 정보(창고시설)</strong></h5>
				...
				<div class="card ml-3 mr-3">
		 		 	<div class="card-body">
						...
						<div class="row mt-3">
							...
							<div class="col-md-2">
								<label for="requsr" style="color:blue">건축구분</label> 
				                 <select class="form-control form-control-sm" id="searchExt6" name="searchExt6" value='<c:out value="${searchVO.searchExt6}"/>'>
										<option value="" <c:if test="${searchVO.searchExt6 eq ''}">selected</c:if>>전체</option>
										<option value="0100" <c:if test="${searchVO.searchExt6 eq '0100'}">selected</c:if>>신축</option>
										<option value="0200" <c:if test="${searchVO.searchExt6 eq '0200'}">selected</c:if>>증축</option>
										<option value="0300" <c:if test="${searchVO.searchExt6 eq '0300'}">selected</c:if>>개축</option>
										<option value="0400" <c:if test="${searchVO.searchExt6 eq '0400'}">selected</c:if>>재축</option>
										<option value="0500" <c:if test="${searchVO.searchExt6 eq '0500'}">selected</c:if>>이전</option>
										<option value="0600" <c:if test="${searchVO.searchExt6 eq '0600'}">selected</c:if>>대수선</option>
										<option value="0700" <c:if test="${searchVO.searchExt6 eq '0700'}">selected</c:if>>용도변경</option>
										<option value="2000" <c:if test="${searchVO.searchExt6 eq '2000'}">selected</c:if>>허가/신고사항변경</option>
										<option value="3000" <c:if test="${searchVO.searchExt6 eq '3000'}">selected</c:if>>가설건축물축조허가</option>
				                 </select>
							</div>
			            </div>			            
			            <hr>
	        ...
	        <div class="col-md-12">
				...
				-->					
							...
							<tbody  align="center">
								...
								<tr>
				  					<td>${status.count} </td> 
				  					<td> <a href="detail?whcode=${basis.registedflg}">${basis.registedflg}</a> </td> 
				  					<td>
				  						<a href="ctrWareForm?fromP=basis&brpk=${basis.mgmPmsrgstPk}" > 
					  						<button type="button" class="btn btn-primary btn-sm">등록</button>
					  					</a>
				  					</td>
				  					<td>
					  					<button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#floorModal" onClick="fn_popFloorInfo('${basis.mgmPmsrgstPk}');">층정보</button>
				  					</td> 
				  					<td>${basis.platPlc}</td> 
				  					<td>${basis.bldNm}</td> 
				  					<td>${basis.archGbCd}</td> 
				  					<td><fmt:formatNumber value="${basis.platArea}" pattern="#,###" /></td> 
				  					<td><fmt:formatNumber value="${basis.archArea}" pattern="#,###" /></td> 
				  					<td><fmt:formatNumber value="${basis.totArea}" pattern="#,###" /></td> 
				  					<td>${basis.archPmsDay}</td> 
				  					<td>${basis.realStcnsDay}</td> 
				  					<td>${basis.useAprDay}</td> 
				<!--  
				  					<td>${basis.mgmPmsrgstPk}</td> 
				  					<td>${basis.bcRat}</td> 
				  					<td>${basis.vlRat}</td> 
